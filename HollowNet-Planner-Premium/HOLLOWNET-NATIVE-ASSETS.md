...
The infrastructure pack at `data/hollownet-infrastructure-assets.json` adds `HNA-26-CTL-003` through `HNA-26-CTL-010` and `HNA-26-CTL-012` through `HNA-26-CTL-015`: the Main Hub, seven additional zone controllers, Pantry and Furnace Room controllers, the Front Gate Controller, and the existing Bridge/Fountain SimFlame Controller. It also contains the planned `HNA-26-OTH-003` Possessed Levitation Effect so that future addition remains distinct from the installed voice-distortion effect. The initially proposed `HNA-26-CTL-011` is retired because Mrs. Grimm already serves as the First Room controller; that ID will not be reassigned.
...
│   └── HNA-26-SCN-001 Mrs. Grimm / First Room Controller
