...
| Queue | `HNA-26-SCN-006` Haunted Radio — Planned | **Confirmed:** Raspberry Pi 5 1GB is the primary controller; it can also run dial, lights and motor commands through interface hardware; ESP32 I/O is optional; `CTL-002` reports through `CTL-004` | Power-on self-test covers sound, lights and motor, then standby until 6:45 PM local. Armed mode uses a randomized timer for startles and otherwise retains the selected station/settings. Hub controls random-delay minimum/maximum plus all settings, schedule, modes, tests, volume, playback and health. Confirm the delay range and self-test feedback. |
...
stateDiagram-v2
    ...
    StartupSelfTest --> Standby: Report sound / lights / motor results
    Standby --> ShowMode: 6:45 PM local
    ShowMode --> ArmedHaunted: Arm startle behavior
    ArmedHaunted --> Startle: Randomized delay
