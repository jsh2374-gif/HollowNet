...
| Exterior Facade | `HNA-26-PRJ-001` Mia & Roy — Installed | **Confirmed:** Pi auto-starts the silent loop; projector starts manually; operation is currently local | Keep the planned direct link to `CTL-004`; no separate facade controller is required. Future Main Hub control and monitoring are desired, but the remote command/status scope still needs definition. |
