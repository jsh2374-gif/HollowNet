...
| Bridge / Fountain | `HNA-26-LGT-001` Jack-O-Lanterns and `HNA-26-CTL-015` SimFlame Controller — Built | **Confirmed:** the SimFlame LV unit drives three 12 V AC flicker circuits | Confirm the existing enable method and whether it remains the local fallback. Decide whether Main Hub needs remote enable, test and off control before linking `CTL-015` to `CTL-004`. |
