...
| Stair Hall | `HNA-26-ACT-001` Stair Hall Ghost and `HNA-26-ANM-005` Filing Cabinet — Installed | **Confirmed independent/local:** actor button triggers the 35-second Pi-based monitor/audio ghost; a cabinet switch commands pneumatic open/close; an under-rug trundle switch triggers the pneumatic file pop-up | Do not add a Stair Hall room controller. Document Ghost reset/startup/re-trigger behavior and Filing Cabinet pneumatic pressure/valve details. HollowNet is documentation/maintenance only. |
