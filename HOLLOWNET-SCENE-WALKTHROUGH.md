...
| Back Hall | `HNA-26-ANM-007` Back Hall Ghost — Installed | **Confirmed local effect:** a dedicated light creates a Pepper's Ghost illusion, while a 12 V motor gives the ghost slight movement | Keep local for now; no room controller is needed. Document light voltage, motor mechanism, power switching, trigger, run pattern and reset behavior. Add reflective surface, light, motor, linkage and mounting checks to maintenance. |
